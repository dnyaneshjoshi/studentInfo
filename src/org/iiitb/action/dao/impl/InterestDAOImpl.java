...
public class InterestDAOImpl implements InterestDAO
{
	private static final String GET_INTERESTS_QUERY=
			"select name, details from interest i, student_interest si where si.student_id=? and i.interest_id=si.interest_id;";
	
	private static final String GET_ALL_INTERESTS_QUERY=
			"select name, details from interest;";
	...
	public List<Interest> getInterests(Connection cn, int studentId) throws SQLException
	{
		...
		while(rs.next())
		{
			ai.add(new Interest(rs.getString("name"),rs.getString("details")));
		}
		rs.close();
		return ai;
	}
